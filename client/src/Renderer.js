import { CANVAS_W, CANVAS_H, GROUND_Y, TILE_W, HEAD_X, SEG_SIZE, SEG_GAP, Y_HISTORY_SIZE, INVINCIBLE_FRAMES, } from '@shared/constants';
const SKY = '#120820';
...
function genCity(seed, count, minW, maxW, minH, maxH, parallax, color, yBase) {
    let r = seed;
...
function drawCity(ctx, layer, scroll) {
...
const STARS = (() => {
    let rs = 999;
    const rng = () => { rs = (rs * 1664525 + 1013904223) & 0xffffffff; return (rs >>> 0) / 0xffffffff; };
...
export class Renderer {
    ...
    render(state, _time) {
        ...
        for (const s of STARS)
            ctx.fillRect(s.x, s.y, s.r + 1, s.r + 1);
        ctx.fillStyle = MOON_COL;
        ctx.beginPath();
        ctx.arc(CANVAS_W - 70, 42, 22, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = SKY;
        ctx.beginPath();
        ctx.arc(CANVAS_W - 62, 38, 17, 0, Math.PI * 2);
        ctx.fill();
        for (const layer of CITIES)
            drawCity(ctx, layer, state.totalScroll);
        this.drawGround(ctx, state);
        for (const f of state.fruits)
            if (!f.collected)
                this.drawFruit(ctx, f, state.tick);
        for (const o of state.obstacles)
            this.drawObstacle(ctx, o);
        if (!flash)
            this.drawSnake(ctx, state);
    ...
    drawGround(ctx, state) {
        for (const t of state.ground) {
            ...
            ctx.fillStyle = GROUND_MID;
            ctx.fillRect(x, GROUND_Y + 3, TILE_W, h - 3);
            ctx.fillStyle = GROUND_TOP;
            ctx.fillRect(x, GROUND_Y, TILE_W, 3);
            ctx.fillStyle = GROUND_LOW;
            ctx.fillRect(x, CANVAS_H - 4, TILE_W, 4);
            ctx.fillStyle = GROUND_LOW;
            ctx.fillRect(x + TILE_W - 1, GROUND_Y + 3, 1, h - 3);
        }
    }
    drawFruit(ctx, f, tick) {
    ...
    drawObstacle(ctx, o) {
        ...
        if (o.type === 'spike') {
            ...
            ctx.fillStyle = SPIKE_DRK;
            ctx.fillRect(cx, o.y, 1, o.h);
        }
        else if (o.type === 'bar') {
            ctx.fillStyle = BAR_DRK;
            ctx.fillRect(x, 0, o.w, o.h);
            ctx.fillStyle = BAR_COL;
            ctx.fillRect(x + 1, 0, o.w - 2, o.h - 2);
            ctx.fillStyle = BAR_DRK;
            ctx.fillRect(x + Math.floor(o.w / 2) - 1, o.h - 4, 3, 6);
        }
        else {
            ...
            ctx.fillStyle = WALL_DRK;
            for (let ry = o.y; ry < o.y + o.h; ry += 5)
                ctx.fillRect(x + 1, ry, o.w - 2, 1);
        }
    }
    drawSnake(ctx, state) {
        ...
        for (let i = segCount - 1; i >= 0; i--) {
            ...
            if (segX < -SEG_SIZE || segX > CANVAS_W)
                continue;
            ...
            ctx.fillStyle = SNAKE_BDR;
            ctx.fillRect(dx, dy, SEG_SIZE, segH);
            ctx.fillStyle = `rgb(26,${g},68)`;
            ctx.fillRect(dx + 1, dy + 1, SEG_SIZE - 2, segH - 2);
        }
        ...
        ctx.fillStyle = SNAKE_BDR;
        ctx.fillRect(hx, hy, SEG_SIZE, headH);
        ctx.fillStyle = SNAKE_HEAD;
        ctx.fillRect(hx + 1, hy + 1, SEG_SIZE - 2, headH - 2);
        if (!state.sliding) {
            ctx.fillStyle = SNAKE_EYE;
            ctx.fillRect(hx + SEG_SIZE - 3, hy + 2, 2, 2);
        }
        if (state.tick % 22 < 11) {
